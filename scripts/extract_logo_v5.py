"""Final logo extraction v5: rows 180-560 contain the logo (AC unit is rows 0-130).
Extract this region, find columns, save as transparent white + navy variants.
"""
...
OUT_DIR = "/home/z/my-project/scripts/pdf_out/logo_final5"
...
# Extract logo region only (rows 170 to 580 to give padding)
logo_top_y = 170
logo_bottom_y = 580
...
# Now within those cols, find rows with white
row_white_in_cols = white_mask[:, logo_left:logo_right+1].sum(axis=1)
sig_rows = np.where(row_white_in_cols > 3)[0]
if len(sig_rows) == 0:
    print("ERROR: No white rows in cols")
    exit(1)
logo_top_rel = sig_rows[0]
logo_bottom_rel = sig_rows[-1]
print(f"Logo rows (relative to logo_region): {logo_top_rel} to {logo_bottom_rel}")
...
# Black version (for very light backgrounds)
