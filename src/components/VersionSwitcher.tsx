...
/**
 * Compact pill-style V1 | V2 switcher for the navbar.
...
export default function VersionSwitcher({ theme = "light" }: { theme?: "light" | "dark" }) {
  ...
  const active = "bg-[var(--accent-amber)] text-[var(--brand-deep)] shadow-[0_2px_8px_rgba(245,166,35,0.4)]";
  ...
  return (
    ...
    >
      <Link href="/" className={`${base} ${!isV2 ? active : inactive}`} aria-current={!isV2 ? "page" : undefined}>
        V1
      </Link>
      <Link href="/v2" className={`${base} ${isV2 ? active : inactive}`} aria-current={isV2 ? "page" : undefined}>
        V2
      </Link>
    </div>
  );
